...
class _ManageStudentsScreenState extends State<ManageStudentsScreen> {
  ...
  final TextEditingController _defaultDeptController = TextEditingController(
    text: 'إعلام تربوي',
  );
  final TextEditingController _defaultLevelController = TextEditingController(
    text: 'الفرقة الثانية',
  );
  ...
  Future<void> _handleSeedData() async {
    ...
    seed.addAll(
      mediaLevel2Students.values.map(
        (s) => {
          'code': s.code,
          'name': s.name,
    ...
    seed.addAll(
      homeEconomicsLevel2Students.values.map(
        (s) => {
          'code': s.code,
          'name': s.name,
          'department': s.department,
          ...
          content: Column(
            ...
            children: [
              DropdownButtonFormField<String>(
                ...
                onChanged: (v) {
                  if (v != null)
                    setSheetState(() => _defaultDeptController.text = v);
                },
              ...
              DropdownButtonFormField<String>(
                ...
                onChanged: (v) {
                  if (v != null)
                    setSheetState(() => _defaultLevelController.text = v);
                },
                decoration: InputDecoration(
                ...
                children: [
                  ...
                  Checkbox(
                    ...
                    onChanged: (v) => setSheetState(
                      () => _overwriteExcelDeptLevel = v ?? true,
                    ),
    ...
    try {
      ...
      if (ext == 'csv') {
        ...
        int idxName = header.indexWhere(
          (h) => h == 'name' || h == 'fullname' || h == 'الاسم',
        );
        int idxCode = header.indexWhere(
          (h) => h == 'code' || h == 'studentcode' || h == 'الكود',
        );
        int idxDept = header.indexWhere(
          (h) => h == 'department' || h == 'القسم',
        );
        int idxLevel = header.indexWhere((h) => h == 'level' || h == 'الفرقة');
        int idxStatus = header.indexWhere(
          (h) => h == 'status' || h == 'الحالة',
        );
        for (int i = 1; i < lines.length; i++) {
          final cols = lines[i].split(',');
          String name = idxName >= 0 && idxName < cols.length
              ? cols[idxName].trim()
              : '';
          String code = idxCode >= 0 && idxCode < cols.length
              ? cols[idxCode].trim()
              : '';
          String department = idxDept >= 0 && idxDept < cols.length
              ? cols[idxDept].trim()
              : '';
          String level = idxLevel >= 0 && idxLevel < cols.length
              ? cols[idxLevel].trim()
              : '';
          String status = idxStatus >= 0 && idxStatus < cols.length
              ? cols[idxStatus].trim()
              : '';
        ...
        final headerRow = table.rows.first
            ...
            .toList();
        int idxName = headerRow.indexWhere(
          (h) => h == 'name' || h == 'fullname' || h == 'الاسم',
        );
        int idxCode = headerRow.indexWhere(
          (h) => h == 'code' || h == 'studentcode' || h == 'الكود',
        );
        int idxDept = headerRow.indexWhere(
          (h) => h == 'department' || h == 'القسم',
        );
        int idxLevel = headerRow.indexWhere(
          (h) => h == 'level' || h == 'الفرقة',
        );
        int idxStatus = headerRow.indexWhere(
          (h) => h == 'status' || h == 'الحالة',
        );
        for (int i = 1; i < table.rows.length; i++) {
          final row = table.rows[i];
          String name = idxName >= 0 && idxName < row.length
              ? (row[idxName]?.value?.toString() ?? '').trim()
              : '';
          String code = idxCode >= 0 && idxCode < row.length
              ? (row[idxCode]?.value?.toString() ?? '').trim()
              : '';
          String department = idxDept >= 0 && idxDept < row.length
              ? (row[idxDept]?.value?.toString() ?? '').trim()
              : '';
          String level = idxLevel >= 0 && idxLevel < row.length
              ? (row[idxLevel]?.value?.toString() ?? '').trim()
              : '';
          String status = idxStatus >= 0 && idxStatus < row.length
              ? (row[idxStatus]?.value?.toString() ?? '').trim()
              : '';
          if (code.isEmpty || name.isEmpty) continue;
          if (overwrite || department.isEmpty) department = defaultDept;
          if (overwrite || level.isEmpty) level = defaultLevel;
      ...
      messenger.showSnackBar(
        SnackBar(
          content: Text(
            'تم استيراد ${students.length} طالبًا',
            style: GoogleFonts.cairo(color: Colors.white),
          ),
          backgroundColor: const Color(0xFF1E8E3E),
        ),
        ...
        content: SingleChildScrollView(
          child: Column(
            ...
            children: [
              ...
              DropdownButtonFormField<String>(
                initialValue: deptController.text.isNotEmpty
                    ? deptController.text
                    : (_departments.isNotEmpty ? _departments.first : null),
                dropdownColor: theme.colorScheme.surface,
                items: _departments
        ...
        actions: [
          ...
          TextButton(
            onPressed: () async {
              ...
              try {
                await ApiService.updateStudent(student.code, {
                                      ...
                                      children: [
                                        ...
                                        Checkbox(
                                          ...
                                          onChanged: (v) async {
                                            ...
                                            try {
                                              await ApiService.setStudentAccess(
                                                student.code,
                                                next,
                                              );
                                              await _loadStudents();
